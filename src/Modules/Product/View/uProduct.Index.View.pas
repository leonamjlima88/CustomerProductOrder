...
uses
  ...
  uYesOrNo.View,
  uAlert.View,
  uConnection.Types, uThreadCustom;
...
begin
  ...
  Try
    ...
    lStoredId := TProductCreateUpdateView.HandleNewRecord;
...
begin
  ...
  Try
    ...
    if not TProductController.Make.Delete(dtsIndex.DataSet.FieldByName('id').AsInteger) then
    begin
...
begin
  ...
  Try
    ...
    // Editar Registro
    if not TProductCreateUpdateView.HandleEditRecord(dtsIndex.DataSet.FieldByName('id').AsInteger) then
      Exit;
...
procedure TProductIndexView.btnNavIndexClick(Sender: TObject);
begin
  inherited;
...
begin
  ...
  // Visualizar Registro
  TProductCreateUpdateView.HandleEditRecord(dtsIndex.DataSet.FieldByName('id').AsInteger, esNone);
end;
...
procedure TProductIndexView.DoSearch(ACurrentPage: Integer);
var
...
begin
  ...
  Try
    ...
    lPageFilter := TPageFilter.Make
      ...
      .AddOrWhere   ('product.id',    coEqual,        String(edtSearchValue.Text).trim)
      .AddOrWhere   ('product.name',  coLikeAnywhere, String(edtSearchValue.Text).trim);

    // Efetuar Pesquisa
    lIndexResult := TProductController.Make.Index(lPageFilter);
...
procedure TProductIndexView.edtNavLimitPerPageExit(Sender: TObject);
begin
  inherited;
  DoSearch;
end;
...
procedure TProductIndexView.edtSearchValueDblClick(Sender: TObject);
begin
  inherited;
  edtSearchValue.Clear;
end;
...
procedure TProductIndexView.mtbIndexAfterOpen(DataSet: TDataSet);
begin
  THlp.FormatDataSet(DataSet);
end;

procedure TProductIndexView.SetLayoutLocate;
var
  lI: Integer;
begin
  FLayoutLocate      := true;
...
class function TProductIndexView.ShowModalLocate: integer;
var
